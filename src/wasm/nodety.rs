...
impl NodetyWrapper {
    ...
    pub fn new() -> Self {
        Self {
            inner: NodetyCached::new(InferenceStep::default_steps()),
        }
    }
    ...
    pub fn update_node(&mut self, node_idx: u32, node: NodeWrapper) -> Result<(), JsValue> {
        self.inner
            .update_node(NodeIndex::new(node_idx as usize), node.0)
            .map_err(|e| JsValue::from_str(&e.to_string()))
    }
    ...
    pub fn add_edge(
        ...
        self.inner
            .add_edge(
                NodeIndex::new(source_node as usize),
                NodeIndex::new(target_node as usize),
                edge.0,
            )
            .map(|idx| idx.index() as u32)
            .map_err(|e| JsValue::from_str(&e.to_string()))
    ...
    pub fn remove_node(&mut self, node_idx: u32) -> Result<(), JsValue> {
        self.inner
            .remove_node(NodeIndex::new(node_idx as usize))
            .map_err(|e| JsValue::from_str(&e.to_string()))
    }
    ...
    pub fn remove_edge(&mut self, edge_idx: u32) -> Option<EdgeWrapper> {
        self.inner
            .remove_edge(EdgeIndex::new(edge_idx as usize))
            .map(EdgeWrapper)
    }
    ...
    pub fn validate(&mut self) -> Vec<crate::wasm::errors::ValidationErrorWrapper> {
        self.inner
            .validate()
            .into_iter()
            .map(crate::wasm::errors::ValidationErrorWrapper)
            .collect()
    }
}
